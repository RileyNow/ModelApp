...
#== perform the upload
start=`date +%s`
response=$(curl -s "${sncUrl}/api/sn_cdm/request/upload-and-commit/component?namePath=${compName}&dataFormat=${dataFormat}&autoDelete=${autoDelete}&appName=$appName" --request PUT --header "Accept:application/json" --header "Content-Type:text/plain" --user ${sncUser}:${sncPwd} --data-binary @testData.json)
end=`date +%s`
